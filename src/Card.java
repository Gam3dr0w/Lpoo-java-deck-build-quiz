public class Card implements SpecialAbility {
    ...
    public void use(Player p, Enemy e, Question q) {
        ...
        switch (id) {
            case "JAVA": e.takeDamage(p.getAttack() + 15 + bonus); break;
            case "LOOP": e.takeDamage((p.getAttack() * 2) + bonus); break;
            case "CURA": p.heal(p.getMaxHp() / 4 + p.getDefense() + bonus); break;
            case "ESCUDO": p.addTemporaryShield(5, 2); break;
            case "BUFF": p.buffAttack(10 + bonus / 2); break;
            ...
            case "MATH": e.takeDamage(30 + bonus); break;
            case "FIREWALL": p.addTemporaryShield(2, 2); p.heal(4 + bonus); break;
        }
    }
